...
class DbHandler {
    ...
    public function getUser($user_id) {
        $stmt = $this->conn->prepare("SELECT user_id, name, email,password, created_at FROM users WHERE user_id = ?");
    ...
    public function addMessage($user_id, $room_id, $message) {
        $response = array();
 
        $stmt = $this->conn->prepare("INSERT INTO messages (room_id, user_id, message) values(?, ?, ?)");
        ...
        if ($result) {
            ...
            $stmt = $this->conn->prepare("SELECT message_id, user_id, room_id, message, created_at FROM messages WHERE message_id = ?");
    ...
    function getChatRoom($chat_room_id) {
        $stmt = $this->conn->prepare("SELECT cr.room_id, cr.name, cr.created_at as chat_room_created_at, u.name as username, c.* FROM chat_rooms cr LEFT JOIN messages c ON c.chat_room_id = cr.chat_room_id LEFT JOIN users u ON u.user_id = c.user_id WHERE cr.chat_room_id = ?");
    ...
    public function getUserByEmail($email) {
        $stmt = $this->conn->prepare("SELECT user_id, name, email,password, created_at FROM users WHERE email = ?");
        $stmt->bind_param("s", $email);
        if ($stmt->execute()) {
            // $user = $stmt->get_result()->fetch_assoc();
